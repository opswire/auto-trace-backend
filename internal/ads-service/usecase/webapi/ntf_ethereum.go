...
func (*NftEthereumWebAPI) GetNftInfo(ctx context.Context, tokenId *big.Int) (*NftInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("ошибка получения URI токена: %v", err)
	}

	resp, err := http.Get(uri)
	if err != nil {
		return nil, fmt.Errorf("ошибка загрузки метаданных: %v", err)
	}
	defer resp.Body.Close()

	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("ошибка чтения метаданных: %v", err)
	}

	var metadata TokenMetadata
	if err := json.Unmarshal(body, &metadata); err != nil {
		return nil, fmt.Errorf("ошибка разбора метаданных: %v", err)
	}
